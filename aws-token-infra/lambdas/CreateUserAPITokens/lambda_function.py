    ...
    if "queryStringParameters" in event and event["queryStringParameters"]:
        if "data" in event["queryStringParameters"]:
            ...
            if len(event["queryStringParameters"]["data"]) == 0:
                ...
                return respond(
                    ValueError("Data parameter given was empty in query string")
                )
            final_resp = {}
            ...
            print(key_resp)
            if (
                "AccessKey" in key_resp
                and "SecretAccessKey" in key_resp["AccessKey"]
                and "AccessKeyId" in key_resp["AccessKey"]
            ):
                final_resp["secret_access_key"] = key_resp["AccessKey"][
                    "SecretAccessKey"
                ]
                ...
                db_response = db.put_item(
                    ...
                    Item={
                        ...
                        "LastUsed": {
                            "N": str(datetime.datetime.utcnow().strftime("%s"))
                        },
                ...
                final_resp["error"] = (
                    "There was an issue creating an AWS ID Token. Please contact an admin."
                )
        ...
        return respond(
            ValueError("No query string found. Please use ?data=<encoded_data>.")
        )
