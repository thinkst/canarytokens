...
class Canarydrop(BaseModel):
    ...
    aws_output: Optional[str] = Field(default=None, alias="output")

    # AWS key and AWS infra stuff
    aws_account_id: Optional[AWSAccountNumber] = None
    aws_region: Optional[AWSRegion] = Field(default=None, alias="region")
    ...
    def _validate_triggered_details(cls, values):
        ...
        if "alert_webhook_url" in values and values["alert_webhook_url"] is not None:
            values["alert_webhook_url"] = str(values["alert_webhook_url"])
